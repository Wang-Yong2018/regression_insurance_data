  ...
  tmp_df <- df |>
    ...
    mutate(
      Annual.Income_cut = cut(
        Annual.Income,
        breaks = result_pct$income_pct,
        include.lowest = TRUE
      ),
      Credit.Score_cut = cut(
        Credit.Score,
        breaks = result_pct$Credit_pct,
        include.lowest = TRUE
      ),
      Health.Score_cut = cut(
        Health.Score,
        breaks = result_pct$Health_pct,
        include.lowest = TRUE
      ),
      Age_cut = cut(Age, breaks = result_pct$age_pct, include.lowest =
                      TRUE)
...
internal_get_weekly_premium <- function() {
  weekly_df <-
    ...
    select(dplyr::all_of(c('date', 'year', 'week', 'wday', 'Premium.Amount'))) |>
