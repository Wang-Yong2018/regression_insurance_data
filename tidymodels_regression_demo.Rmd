...
```
### -v1.02 - add grp_col featuure which as seqency id 
```{r}

get_vmod_v1.02 <- function(nrows = Inf) {
  df <-get_train()
...
# vmod_v1.02 <- get_vmod_v1.02()
```
### -v1.03 - add grp_col featuure and seconds features add microsecond feature
```{r}

get_vmod_v1.03 <- function(nrows = Inf) {
  df <-get_train()
...
vmod_v1.03 <- get_vmod_v1.03()
```


### -v1.03_b - basedon v1.03 and revise dummy feature
```{r}

get_vmod_v1.03_b <- function(nrows = Inf) {
  df <-get_train()
  ...
  vmod <-
    get_fit_wf(
      ...
      data = df,
      name = current_function_name,
      description = 'v1.03_b add grouped feature & seconds feature',
      is_log1py = TRUE,
      return_wf = TRUE
    )
  return(vmod)
...
# TODO is there pattern that, group by high price insuance first and then buy low price. later. It may related to time series .2019-early of 2020 is very high.  and people
...
show_model_permformance() %>% 
  ...
  scale_y_continuous(limits=c(1.0,2.10),
...
```{r}
  ...
  get_mods_log_residual(mod_name_pattern = 'get_vmod_v0.9|v1.03')|>
