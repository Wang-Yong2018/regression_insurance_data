...
#    step_time(date, features= c('second'))|>
    ...
    step_impute_median(all_numeric_predictors())|>
    step_unknown(all_nominal_predictors()) |>
    step_cut(Annual.Income,breaks=income_pct,include_outside_range=TRUE)|>
    step_cut(Age,breaks=age_pct,include_outside_range=TRUE)|>
    step_mutate(
      date_year = as_factor(date_year),
      Insurance.Duration = as_factor(Insurance.Duration),
      Previous.Claims = as_factor(Previous.Claims),
...
```{r}
  ...
  get_mods_log_residual(mod_name_pattern = 'vmod_v0.10|vmod_v0.8')|>
