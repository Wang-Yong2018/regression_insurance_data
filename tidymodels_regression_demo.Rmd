...
```
### -v1.03l - add grp_col featuure and seconds features add microsecond feature
```{r}

get_vmod_v1.03l <- function(nrows = Inf) {
  df <-get_train()
...
#    step_corr(all_numeric_predictors(), threshold = 0.8)|>
  ...
  vmod <-
    get_fit_wf(
      ...
      data = df,
      name = current_function_name,
      description = 'v1.03 add grouped feature & seconds feature',
      is_log1py = TRUE,
      return_wf =FALSE 
    )
  return(vmod)
...
```{r}
  ...
  get_mods_log_residual(mod_name_pattern = 'v1.04|v1.1')|>
...
save_submit <-function(best_mod ){
  ...
  submit_file_name <- 'output/sample_submit.csv_v1.04_microsec.gz'
